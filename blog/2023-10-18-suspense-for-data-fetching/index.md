...
비동기 상태에 따라 컴포넌트를 노출하는 UX/UI 설계는 많은 고민을 필요로 합니다. 이러한 문제를 해결하기 위해 React는 다양한 방법을 제공하는데, 그 중 Suspense와 Error Boundary를 활용한 선언적 데이터 패칭이 있습니다. 이 방법은 코드의 가독성과 유지보수 측면에서 매우 효과적입니다.

이 포스팅에서는 Suspense와 Error Boundary를 활용한 선언적 데이터 패칭이 어떻게 효과적인지 함께 알아봅시다.
...
전통적으로, 컴포넌트 내부에서 data, loading, error의 상태를 사용하여 데이터 패칭 처리가 이루어졌습니다. 아래의 예시 코드는 비동기 요청으로 데이터를 받아와 사용자에게 전달하는 간단한 컴포넌트입니다.
...
const SampleContents = () => {
  ...
  return (
    <div>
      {sampleDatas.map((data) => <Content data={data} />)}
    </div>
  )
}
...
선언적 데이터 패칭은 React의 주요 개념인 '선언형 프로그래밍'에 기반합니다. 이는 상태의 변화에 따라 UI를 직접 조작하지 않고, 어떤 상태에 따라 어떤 UI가 보여져야 하는지만 집중합니다. 이러한 접근법은 비동기 작업의 복잡성을 크게 줄여주며 코드의 가독성과 유지보수성을 향상시킵니다. Suspense와 Error Boundary는 이러한 선언형 프로그래밍 방식으로 비동기 작업과 에러 처리를 담당합니다.
...
[Error Boundary](https://react.dev/reference/react/Component#catching-rendering-errors-with-an-error-boundary)는 하위 컴포넌트 트리에서 발생하는 에러를 캡처하여 대체 UI를 보여주거나 에러 리포팅 등의 작업을 수행합니다. 필요에 따라 커스터마이징을 할 수 있는데, 다음 기회에 다른 포스팅으로 진행해보겠습니다.
...
useQuery를 Suspense, Error Boundary와 함께 사용하려면 `suspense: true` 옵션을 넣어주면 됩니다. 모든 쿼리문에서 사용할 것 이라면 `QueryClient`를 생성할때 기본 옵션으로 지정해 줄 수 있습니다.
...
또한, Error Boundary를 상위 컴포넌트에서 공통으로 처리할 수 있습니다.
...
Suspense를 사용할 때 한 컴포넌트 내에서 useQuery를 사용하는 경우 비동기 요청 Waterfall이 발생합니다.
...
최신 버전의 React Query를 사용한다면 `useQuery`대신 `useQueries`의 사용을 고려해 볼 만 합니다. 비동기 요청을 병렬로 처리해 줍니다!

### Loader와 Skeleton의 노출로 인한 사용자 경험
...
지금까지 Suspense와 Error Boundary를 활용한 선언적 데이터 패칭에 대해 알아봤습니다.

Suspense와 Error Boundary를 활용한 비동기 처리는 컴포넌트의 가독성, 유지보수 측면, 비즈니스 로직 분리 측면에서 상당한 이점이 있지만, 무분별한 Suspense, Error Boundary는 심각한 성능 저하, 사용자 경험 저하를 일으킬 수 있기 때문에, Suspense, Error Boundary의 장점과 단점을 좀 더 고민하고 동작 원리와 적절한 사용성에 대해 탐구해야 될 것 같습니다.
