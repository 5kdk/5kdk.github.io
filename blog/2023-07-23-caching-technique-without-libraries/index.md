...
이 포스팅에서는 서버 데이터에 대한 캐싱 기능을 라이브러리 없이 직접 구현해보면서 이해해본다.
...
두 라이브러리의 컨셉은 조금 다르지만, 서버 데이터에 대해 캐싱하고 효율적인 서버 데이터 요청을 추구한다는 점이 같다. 캐싱 라이브러리에 대한 포스팅이 아니기에, 본 포스트에서는 자세하게 다루진 않겠다.~~(가장 큰 차이점은 전역 상태관리로서의 사용 그리고 서버로의 mutation 여부다.)~~
...
- cache time 이내의 서버 요청 호출 방지 (캐시 타임 비교 로직)
...
- 객체를 컨트롤할 수 있는 직관적인 프로토타입 메서드들을 제공(ex: `has()`, `set()` 등)
- 후술할 테 지만 추가 기능을 고려해 삽입 순서가 보장되는 `Map`을 선택했다.

---

### 캐싱 커스텀 훅 구현

React-Query의 `useQuery`의 이름을 본따 `useCacheQuery`라는 커스텀 훅을 만들었다. 초기에 관심사 분리를 고려하여 필요 로직들만 정리할 것이다.

`cacheStore` 객체를 모듈스코프에 선언함으로써 `useCacheQuery`를 사용하는 모두가 접근할 수 있다.

즉, `useCacheQuery`를 어디서 호출하든 `cacheStore`를 공유한다는 것이다.  
...
데이터 요청에 대한 상태에 따라 컴포넌트에서 조건부 렌더링으로 처리할 수 있도록 `isLoading`과 `error`에 대한 상태도 함께 캡슐화한다.
...
서버에서 받아온 데이터는 `cacheStore`에 `{ data: 서버에서 가져온 데이터, createAt: 현재 시간 }` 의 형태로 저장한다. 현재 시간을 함께 저장하는 이유는 캐시 타임과 비교를 위함이다.
...
그렇담 우리의 캐시 확인 과정은 어떻게 이루어질까? 함수 로직에 캐시 확인 과정을 한 스푼 얹어보자.
...
자동완성 검색 기능에 이 훅을 적용해보자. `getSearchTerms`는 `input`을 받아 파라미터로 전달해서 서버에 데이터 요청을 하는 비동기 함수다. (`input` 상태 대한 디바운싱 처리와 텍스트 전처리로 캐시에 불필요한 요청이 들어가지 않도록 처리해두었다. 컴포넌트의 상세 코드는 생략한다.)
...
같은 검색어를 세차례 작성했지만 요청은 1번만 일어났다. `cacheStore`에 제대로 들어가 있는 것을 보니 동작하는 것을 확인했다.
...
필자는 최대 캐시의 개수를 설정하고 이를 넘을시 오래된 캐시부터 지우면 될 것이라 생각했다.
...
`cacheStore`가 `maxCacheSize`의 개수를 넘는다면 오래된 캐시를 삭제해주는 로직을 추가해주자. `Map`은 삽입된 순서를 기억하므로 첫번째 캐시를 지워주면 된다!
...
지금 상태에서 받아온 데이터를 컴포넌트에서 가공하여 사용할 수 있다. 지금까지 구현한 커스텀 훅에 `select` 옵션을 만들어서 우아하게 데이터를 가져와 보면 좋을 것 같았다.
...
서버에 POST, PUT, DELETE 등 요청을 보낼 수 있는 mutation 기능과 선언적으로 에러, 로딩 처리를 할 수 있도록 `ErrorBoundary`와 `Suspence`를 지원하는 기능까지 추가하면 좋을 것 같다.
