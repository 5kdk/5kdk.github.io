...
두 라이브러리의 컨셉은 조금 다르지만 서버 데이터에 대해 캐싱하고 효율적인 서버 데이터 요청을 추구한 다는점이 같다. 캐싱 라이브러리에 대한 포스팅이 아니기에, 본 포스트에서는 자세하게 다루진 않겠다. ~~(가장 큰 차이점은 전역 상태관리로서의 사용 그리고 서버로의 mutation 여부다.)~~
...
React-Query의 `useQuery`의 이름을 본따 `useCacheQuery`라는 커스텀 훅을 만들자. 초기에 관심사 분리를 고려하여 필요 로직들만 정리할 것이다.
